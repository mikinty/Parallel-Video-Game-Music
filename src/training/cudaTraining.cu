...
__global__ void CountSection(deviceS, sLength, deviceB, bLength, deviceHigh, deviceLow, deviceChord)
{
    sound_t* part;
    int start;
    int end;
    ...
    for (int noteIndex = start; noteIndex < end; noteIndex++){    	
        ...
        curTone = part[noteIndex].noteNumber;
        curLength = part[noteIndex].noteLength;

        int cell;
        if (curTone > NUM_TONES) { //insert into chord matrix
            cell = findChordCell(curTone, prevTone1);
            atomic_add(deviceChord[cell]);
        }
